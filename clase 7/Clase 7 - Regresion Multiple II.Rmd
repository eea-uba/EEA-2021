...
Habíamos realizado tres modelos distintos para tratar de explicar el precio de las propiedades: 
...
tidy_sc_r <- tidy(modelo_st_r, conf.int = TRUE)
tidy_sc_r
...
```
En este gráfico podemos observar los coeficientes estimados para este modelo con sus respectivos intervalos de confianza. Tanto el gráfico como la salida del modelo tidy se puede apreciar que el intervalo de confianza (IC) del 95% de las variables no contienen al 0. Es decir, que tanto la superficie como las habitaciones resultan significativas para explicar el precio. 
...
En este caso se observa también que todas las variables resultan estadísticamente significativa para explicar al precio. Esto se ve reflejado en que el intervalo de confianza del 95% no contiene el cero.
...
tidy_sc_l3
# Plot de los Coeficientes
ggplot(tidy_sc_l3, aes(estimate, term, xmin = conf.low, xmax = conf.high, height = 0)) +
  ...
  labs(y = "Coeficientes β", x = "Estimación")
```
En este caso se observa que no todas las categorías de barrios resultan estadísticamente significativa para explicar al precio (p-valor > 0.05) y como se verifica a través de los IC que algunos barrios (niveles de la variable l3) incluyen al 0 y otros no.  
...
models <- list(modelo_sc_r = modelo_sc_r, modelo_sc_pt = modelo_sc_pt, modelo_sc_l3 = modelo_sc_l3, modelo_varias = modelo_varias, modelo_varias_l3 = modelo_varias_l3)
...
Como no todos los modelos tienen igual número de variables, debemos compararlos a través del $R^2_a$, ya que sino podemos incurrir en un error. Por ejemplo, vemos que en los modelos que incluyen a l3, el $R^2$ es el mayor al $R_a^2$. Esa diferencia está explicada por el mayor número de dummies que contienen. A pesar de ello los mejores modelos resultan ser los que incluyen la variable barrios ya que poseen mayor $R_a^2$.

Analizando el R2 ajustado, el **mejor modelo** resulta ser modelo_varias_l3: que explica el precio en función de superficie cubierta, descubierta, barrios, habitaciones y tipo de propiedad. Este modelo explica 74,27% de la variabilidad del precio, es decir, más que todos los restantes modelos. 
...
Podemos obtener los valores del RMSE en el dataset de **training** para los cinco modelos evaluados usando `map`
...
Creamos una lista de 5 dataframes con los valores predichos. Luego podemos aplicar la función para calcular el RMSE de los cuatro modelos en el set de **training**
