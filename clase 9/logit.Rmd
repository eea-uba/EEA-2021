...
La **regresión logística** es útil para problemas de predicción de clases. En este caso vamos a tratar de resolver el problema de predecir si una persona que viajaba a bordo del Titanic sobrevivió o no. 

## Levantamos el conjunto de datos

Este conjunto de datos proviene de [Kaggle - Titanic: Machine Learning from Disaster](https://www.kaggle.com/c/titanic/overview).
...
g <- ggpairs(dftitanic_train %>% 
               select("Survived","Pclass", "Sex", "Age", "Fare", "SibSp", "Parch"),
             title = "Correlograma de variables",
...
Parece tener bastantes problemas para estimar la probabilidad de supervivencia de los individuos. 
...
```{r}
(models %>% 
  filter(models %in% c('class')) %>% mutate(tidy = map(mod, tidy)) %>%  unnest(tidy) %>% 
  filter(term == 'Pclass2') %>% select(estimate))$estimate
```
...
* β0 =  corresponde a personas que viajaron en primera clase.

* β1 y β2 corresponden a personas que viajaron en la segunda y tercera clase, respectivamente. El coeficiente estimado en ambos casos fue negativo, indicando que la probabilidad de supervivencia disminuye en comparación con la primera clase.  Al ser más negativo el coeficiente estimado de Pclass3, se espera que la probabilidad de Supervivencia sea menor aún para esta clase que para un pasajero en clase 2.

**Modelo sex**

* β0 corresponde a mujeres que viajaron a bordo del titanic.

* β1 representa a los hombres que viajaron en el titanic e indica que **la probabilidad de supervivencia se reduce** en comparación a las mujeres.

**Modelo age**

* β1 indica que la probabilidad de supervivencia disminuye por cada año más de edad de la persona.


Para la variable Pclass, observamos los coeficientes de Pclass2 y Pclass3. En ambos casos los valores resultan negativos, indicando que si el pasajero pertenece a la Clase 2 o Clase 3, su probabilidad de Superviviencia (Survived) esperada disminuye respecto de una persona que no pertenece a estas clases, controlando por las otras variables del modelo (Sex y Age). En particular,

Para la variable Sex observamos el coeficiente de SexMale negativo; esto indica que cuando una persona es de Sexo Masculino, su probabilidad de Supervivencia esperada disminuye respecto de una persona de sexo femenino, controlando por las otras variables del modelo.

Para la Age el coeficiente estimado resulta negativo, indicando que ante un aumento en la edad del Pasajero, su probabilidad de supervivencia esperada disminuye, controlando por las otras variables del modelo.
