...
Se transforman las variables **Survived**,  **Pclass** y **Embarked** a factor.
...
ggplot(distrib, aes(x = Survived, y = prop, fill = Survived, label = prop)) + 
...
g <- ggpairs(dftitanic_train %>% 
               select(where(is.numeric)) %>% select(-c(PassengerId)),  
